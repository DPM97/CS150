...
public class MatrixUndirectedWeightedGraph extends UndirectedWeightedGraph {
    ...
    NamedEdge[] getEdgeNames() {
        ArrayList<NamedEdge> strArr = new ArrayList();
        int covered = 0;
        for (int i = 0; i < this.nodes.length; i++) {
            for (int j = 0; j < this.nodes[i].length; j++) {
                if (this.nodes[i][j] != null) {
                    strArr.add(new NamedEdge((String) this.verticies.get(i), (String) this.verticies.get(j), this.nodes[i][j]));
                    covered++;
                }
            }
        }
        NamedEdge[] arr = new NamedEdge[strArr.size()];
    ...
    NumberedEdge[] getEdgeNumbers() {
        ArrayList<NumberedEdge> intArr = new ArrayList();
        int covered = 0;
        for (int i = 0; i < this.nodes.length; i++) {
            for (int j = 0; j < this.nodes[i].length; j++) {
                if (this.nodes[i][j] != null) {
                    intArr.add(new NumberedEdge(i, j, this.nodes[i][j]));
                    covered++;
                }
            }
        }
        NumberedEdge[] arr = new NumberedEdge[intArr.size()];
        for (int i = 0; i < arr.length; i++) {
            arr[i] = intArr.get(i);
        }
        return arr;
